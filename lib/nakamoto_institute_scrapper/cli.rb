...
class NakamotoInstituteScrapper::CLI
  ...
  def menu_list_of_documents
    ...
    if input.to_i > 0 && input.to_i <= NakamotoInstituteScrapper::Document.all.size
      document = NakamotoInstituteScrapper::Document.all[input.to_i - 1]
  ...
  def display_documents
      NakamotoInstituteScrapper::Document.all.each_with_index do |document, index|
  ...
  def new_request?
    ...
    case user_input
    when "y"
      NakamotoInstituteScrapper::Document.reset
      true
    when "n"
      false
